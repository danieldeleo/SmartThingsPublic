...
preferences {
    ...
    section("Don't notify 5 hours past this time"){
    	input "silentTime", "time" 
    }
}
...
def meterHandler(evt) {
	...
	if(meterValue == 0 && state.running) {
        ...
        def localSilentTime = timeToday(silentTime, location.timeZone).time
        ...
        if (switches) {
            ...
            else if(now() < localSilentTime || now() > localSilentTime + (5*60*60*1000)) {
                flashLights()
            }
        }
    }
